...
// Function to add delete comment notification
async function deleteCommentNotification(postId, currentUser, commentContent) {
  try {
    const post = await db.collection('posts').findOne({ _id: new ObjectId(postId) });
    if (!post) {
      throw new Error('Post not found');
    ...
    const notification = {
      user_id: post.user_id,
      notification_content: `${currentUser.user_name} deleted a comment on your post: ${commentContent}`,
      seen: false,
      date: new Date()
    };
...
app.post('/delete-comment', async (req, res) => {
  const { post_id, comment_id, user } = req.body;
  ...
  try {
    ...
    await deleteCommentNotification(post_id, user, comment.comment_content);
...
app.post('/fetch-notifications', async (req, res) => {
  ...
  try {
    const notifications = await db.collection('notifications').find({ user_id: new ObjectId(user_id) }).toArray();
