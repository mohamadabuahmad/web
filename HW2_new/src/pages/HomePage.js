...
function HomePage() {
  ...
  const enrichPostsData = async (posts, comments, likes) => {
    const enrichedPosts = await Promise.all(posts.map(async (post) => {
      ...
      const postComments = await Promise.all(comments.filter(comment => comment.post_id === post._id.toString()).map(async (comment) => {
        ...
        return new Comment(comment.post_id, comment.user_id, comment.comment_content, comment._id.toString(), commentUsername);
      }));
  ...
  const removeLike = async (postId) => {
    try {
      ...
      if (response.data.message.includes('successfully')) {
        const updatedPosts = posts.map(post => {
          if (post.post_id === postId) {
            post.likes = post.likes.filter(like => like.user_id !== currentUser.user_id);
            post.likes_num -= 1; // Update the likes count
          }
          return post;
        });
  ...
  const deleteComment = async (postId, commentId) => {
    try {
      const response = await axios.post('/delete-comment', { post_id: postId, comment_id: commentId, user: currentUser });
  ...
  const CommentSection = ({ post, addComment, addLike, removeLike, deleteComment, currentUser }) => {
    ...
    const [userHasLiked, setUserHasLiked] = useState(post.likes.some(like => like.user_id === currentUser.user_id)); // Initial state
